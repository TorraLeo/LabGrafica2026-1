...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		color = glm::vec3(0.40f, 0.47f, 0.54f);
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.530f, 0.810f, 1.0f);
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.870f, 0.190f, 0.390f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo y pir�mide triangular
		...
		model = glm::scale(model, glm::vec3(1.0f, 5.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.550f, 0.710f, 0.000f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo y pir�mide triangular

		// ------------------------ARTICULACION 4 (Punto de anclaje de la canasta) ------------------------
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, -2.5f, 0.0f));
		float angulo4 = mainWindow.getarticulacion4();
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		sp.render();

		// ------------------------CANASTA------------------------
		glm::vec3 canastaPosicion = glm::vec3(model[3]); // Extraemos la posición final de la cadena de transformaciones (la posición de la articulación 4)
		glm::mat4 canastaModel = glm::mat4(1.0f); // Creamos una matriz nueva para la canasta, ignorando las rotaciones heredadas
		canastaModel = glm::translate(canastaModel, canastaPosicion); // La movemos a la posición correcta
		canastaModel = glm::rotate(canastaModel, glm::radians(angulo4), glm::vec3(1.0f, 0.0f, 0.0f));
		canastaModel = glm::scale(canastaModel, glm::vec3(2.0f, 2.0f, 2.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(canastaModel));
		color = glm::vec3(0.40f, 0.47f, 0.54f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[0]->RenderMesh();
